...
- **双启动模式支持**：
  ...
  - **Hook 注入模式（DLL Hook）**：通过环境变量注入直接指定数据目录，免去软链接重指向，支持多账户并行运行。
- **降级保护机制（Hook Fallback）**：Hook 启动失败时可自动平滑降级为软链接模式启动。
- **多账户并行启动**：通过 `-s tag1,tag2` 一次启动多个账户，各自独立运行互不干扰（Hook 模式）。
- **单实例与 IPC 通信**：支持单实例锁防止重复启动；多实例时通过命名管道（IPC）将切换指令转发至主进程处理。
- **窗口智能置顶**：Hook 模式下检测到目标账户已在运行时，自动激活并置顶已有窗口，避免重复启动。
...
打包完成后，可执行文件位于 `output/` 目录，文件名包含版本号、平台与工具链信息。
...
| --switch [TAG]   | -s [TAG] | 切换并启动指定账户（支持逗号分隔多账户）            | `TAS.exe -s tag1,tag2`           |
...
> **多账户启动**：`--switch` 参数支持逗号分隔的多个标签（如 `-s work,personal`），Hook 模式下将同时启动多个 Telegram 实例。单实例模式开启时禁止多账户启动。
...
| 配置项 | 类型 | 默认值 | 说明 |
|-------|------|-------|------|
| `launch_mode` | string | `"symlink"` | 启动模式，可选 `symlink`（符号链接模式）或 `hook`（DLL Hook 注入模式） |
| `hook_fallback` | bool | `true` | Hook 模式启动失败时，是否自动降级为软链接模式 |
| `single_instance` | bool | `false` | 单实例模式，防止重复启动 TAS；多实例时通过 IPC 将切换指令转发至主进程 |
| `isolate_appid` | bool | `false` | AppID 隔离开关，用于多实例窗口分组隔离 |

## 注意事项与故障排除

1. **调试日志**：默认仅输出 `INFO` 级日志，如遇启动或切换问题，可添加 `--debug` 参数运行查看详细跟踪信息或检查 `TAS.log`。
2. **符号链接权限**：软链接模式创建符号链接需要系统权限，建议以管理员身份运行程序或在 Windows 设置中启用"开发者模式"。
3. **Hook 模式**：Hook 模式适用于不想频繁修改 `tdata` 软链接的场景，支持多账户并行运行；若 Hook 注入被安全软件拦截，可开启 `hook_fallback` 自动降级。
4. **单实例与多开**：`single_instance` 开启后，重复启动 TAS 会将切换指令通过 IPC 管道转发给已运行的主进程；关闭则允许各自独立运行。单实例模式下禁止多账户同时启动。
5. **窗口置顶**：Hook 模式下若目标账户已在运行，TAS 会自动激活并置顶该窗口而非重复启动。
