...
from src.core.constants import IDENTITY_FOLDER, INFO_SUBFOLDER, KEY_FOLDER, TAG_FILE, TELEGRAM_IDENTITY_KEY
...
class EditLabelDialog(QDialog):
    ...
    def set_default_and_accept(self) -> None:
        """标记该账户为默认账户并接受对话框."""
        if self.validate_inputs():
    ...
    def auto_fill_from_folder(self, folder: str) -> None:
    ...
    def show_keys_dialog(self) -> None:
        """展示密钥查看详细弹窗."""
...
class ShowKeyDialog(QDialog):
    ...
    def __init__(self, info: str = "", identity: str = "", key: str = "", parent: Optional[QWidget] = None) -> None:
        """初始化密钥明细对话框并填充密钥值."""
        super().__init__(parent)
